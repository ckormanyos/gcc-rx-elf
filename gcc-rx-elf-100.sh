...
#
# Example call(s):
#   ./gcc-rx-elf-100.sh 13.2.0 x86_64-linux-gnu x86_64-linux-gnu
#   ./gcc-rx-elf-100.sh 13.2.0 x86_64-w64-mingw32 x86_64-w64-mingw32 /c/mingw
#
...
mkdir objdir-binutils-2.41-rx-elf-gcc-"$MY_VERSION"
cd objdir-binutils-2.41-rx-elf-gcc-"$MY_VERSION"
../binutils-2.41/configure --prefix=$SCRIPT_DIR/local/gcc-"$MY_VERSION"-rx-elf --target=rx-elf --enable-languages=c,c++ --build="$BUILD_NAME" --host="$HOST_NAME" --with-pkgversion='Built by ckormanyos/real-time-cpp' --disable-plugins --enable-static --disable-shared --without-system-zlib --with-newlib --disable-libgomp --disable-libada --disable-libssp --disable-__cxa_atexit --disable-threads --disable-nls --disable-win32-registry --disable-sjlj-exceptions --with-dwarf2 --disable-libquadmath --disable-fixed-point --disable-decimal-float --with-isl=$SCRIPT_DIR/local/isl-0.24 --with-cloog=$SCRIPT_DIR/local/cloog-0.18.1 --with-gmp=$SCRIPT_DIR/local/gmp-6.3.0 --with-mpfr=$SCRIPT_DIR/local/mpfr-4.2.1 --with-mpc=$SCRIPT_DIR/local/mpc-1.3.1 --with-libiconv-prefix=$SCRIPT_DIR/local/libiconv-1.17 --with-zstd=$SCRIPT_DIR/local/zstd-1.5.5/lib --disable-werror
make --jobs=6
make install
echo


ls -la $SCRIPT_DIR/local/gcc-"$MY_VERSION"-rx-elf/bin
ls -la $SCRIPT_DIR/local/gcc-"$MY_VERSION"-rx-elf/bin/rx-elf-ld*
result_binutils=$?
...
cd ..
mkdir objdir-gcc-"$MY_VERSION"-rx-elf
cd objdir-gcc-"$MY_VERSION"-rx-elf
...
ls -la $SCRIPT_DIR/local/gcc-"$MY_VERSION"-rx-elf/bin
ls -la $SCRIPT_DIR/local/gcc-"$MY_VERSION"-rx-elf/bin/rx-elf-gcc*
result_gcc=$?
